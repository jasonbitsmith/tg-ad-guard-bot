...
2026-09-23 已部署 v2.1.1。该版本在后台增加按群设置的欢迎语、群规、链接域名黑白名单与关键词命中统计；确认的收米日薪、拍照日结批量广告会直接永久封禁；Cloudflare 的 `guard-v2` 存储迁移和原有密钥绑定均保留。
...
- 黑名单关键词首次命中即删消息；请只将明确禁止的广告词加入本群词库。
- 未命中黑名单词的低风险内容仅记录；中风险（评分 4–6）删消息并计一次警告。
- 高风险（评分 >= 7）或达到 3 次警告，在超级群临时禁言 10 分钟。
- “收米 + 日薪承诺”与“拍照兼职 + 日结”模板会删消息并直接永久封禁；同一短文由多个账号重复发布也会触发群级刷屏处理。
- 除上述已确认模板外，不自动永久封禁。普通群不支持临时禁言，降级为删消息、计警告。
